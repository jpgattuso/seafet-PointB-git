...
y <- read.table("../../pCloud Sync/exp153_carbonates_point_B/data/Data_DIC.csv", header=T, sep=",", dec=".", as.is=T)
...
#For discrete plot
discrete_p <- y %>%
  dplyr::select(date, S_ptb, T_ptb, at_ptb, dic_ptb, PO4_ptb, Si_ptb)
discrete_p <- melt(discrete_p, id.vars="date")

time_series_dis <- ggplot(discrete_p) + geom_point(aes(x=date, y=value, color=variable),size=0.5) +
...
```{r discrete table, echo=FALSE, warning= FALSE, message=FALSE, out.width="100%"}
...
```{r discrete temperature T_seaF VS T_ptb, echo=FALSE, message=FALSE, out.width="70%", fig.align='left'}
# Regression model II
## 1
## T_seaF VS T_ptb
...
calib_spectro_ptb <- sf_calib(calEint=discrete_ptb$Eint, calEext=discrete_ptb$Eext, calpH=discrete_ptb$pHspec_Tptb, calT=discrete_ptb$T_ptb, calSal=discrete_ptb$S_ptb)
...
# on discrete samples
pHT_calib_ptb_discrete <- sf_calc(calEint=discrete_ptb1$Eint, calEext= discrete_ptb1$Eext, E0int25= discrete_ptb1$E0int25, E0ext25= discrete_ptb1$E0ext25, calT= discrete_ptb1$T_ptb, calSal= discrete_ptb1$S_ptb)
...
pHT_calib_ptb <- sf_calc(calEint=seafet_ptb$Eint, calEext= seafet_ptb$Eext, E0int25= seafet_ptb$E0int25, E0ext25= seafet_ptb$E0ext25, calT = seafet_ptb$T_ptb, calSal= seafet_ptb$S_ptb)
...
```{r POINT B deltas plots, echo=FALSE, warning = FALSE, message = FALSE, out.width='100%'}
...
delta_corr_xts <- dplyr::select(ddl, date, delta_calc15_corr, delta_spec15_corr)
delta_corr_xts <- as.xts(delta_corr_xts, order.by=delta_corr_xts$date)

dygraph(delta_corr_xts, ylab = "Delta pH<sub>T</sub> 15°C") %>%
  dySeries("delta_calc15_corr", drawPoints = TRUE, pointSize=4, label = "seaFET corr - calculated")%>%
  dySeries("delta_spec15_corr", drawPoints = TRUE, pointSize=4, label = "seaFET corr - spectro") %>%
  dyLimit(0)%>%
...
> McLaughlin et al. 2017 [Article](https://reader.elsevier.com/reader/sd/pii/S2352485516302559?token=32EEF36D3E70FC7ADF8BD0915A7FDBC9DC3EBEA64EFF1B987B3FF82AEEEAFC5F8DA33DB2A977351D9D78CAED13C16A52)
>
...
>
>“co-deployment of at least one other independent sensor would provide confirmation that the sensors are not drifting and greatly improve confidence in
data quality.”
...
```{r plots pH seaFET15 vs calc15, echo=FALSE, warning = FALSE, message = FALSE, out.width='70%'}
########### seaFET CORRECTED
...
<br>
```{r plots pH seaFET 15 vs spectro 15, fig.keep='all', echo=FALSE, warning = FALSE, message = FALSE, out.width='70%'}
########### seaFET CORRECTED
titre <- "seaFET data corrected"
...
## Comparaison between Point B pH calculated (SNAPO) / pH spectro

```{r Comparaison EOL/PtB, echo=FALSE, include = FALSE,warning = FALSE, message = FALSE, out.width="100%"}
comparaison <- read.table("../../pCloud Sync/exp153_carbonates_point_B/data/Data_DIC.csv", header=T, sep=",", dec=".", as.is=T)
...
comparaison <- comparaison %>%
  dplyr::select(date, depth,temp_field, salinity_field, temp_lab_ptb, pH_s_ptb_temp_lab, qflag_pH_s_ptb,temp_lab_eol, pH_s_eol_temp_lab, qflag_pH_s_eol, ta, dic,PO4, SIOH4)#%>%
...
# select only days were both pH spec and ta/dicB are present with no NA to campare  
comparaison <- comparaison %>%
  dplyr::select(date, depth,temp_field, salinity_field,temp_lab_ptb, pH_s_ptb_temp_lab,qflag_pH_s_ptb,ta, dic,PO4, SIOH4)%>%
  dplyr::filter( depth == 0 )%>%
dplyr::filter(!is.na(temp_field))
...
ptb$pH_s_ptb_Tptb <- pHinsi(pH=ptb$pH_s_ptb_temp_lab,ALK=ptb$ta*1e-6, Tinsi=ptb$temp_field, Tlab=ptb$temp_lab_ptb, S=ptb$salinity_field,Pt=ptb$PO4,Sit=ptb$SIOH4)
#eol <- eol %>% 
...
ptb <- ptb %>%
  ...
  dplyr::summarize(pH_s_ptb_Tptb = mean(pH_s_ptb_Tptb, na.rm = T),
                  ...
                  salinity_field = mean(salinity_field, na.rm = T),
                   at_ptb = mean(ta, na.rm = T),
                   dic_ptb = mean(dic, na.rm = T),
                  PO4 = mean(PO4, na.rm = T),
                  SIOH4 = mean(SIOH4, na.rm = T))
 # eol <- eol %>%
 #  dplyr::group_by(date)%>%
...
# avec T, S ptB et AT DIC ptB
carb <- carb(15, ptb_noNAs$at_ptb*0.000001, ptb_noNAs$dic_ptb*0.000001, S=ptb_noNAs$salinity_field, T=ptb_noNAs$temp_field, P=0, Pt=ptb_noNAs$PO4*1e-6, Sit=ptb_noNAs$SIOH4*1e-6 ,k1k2="x", kf="x", ks="d", pHscale="T", b="l10")
ptb_noNAs$pHcalc_ptB<- carb$pH


# Regression Model II
# Plot
titre <- "Calc pH (SNAPO AT/CT) vs Spec pH (POINT B)"
fit <- lmodel2(data = ptb_noNAs%>%filter(date != "2018-01-18"), pHcalc_ptB ~ pH_s_ptb_Tptb, nperm = 99)
...
comparaison_p <-  
  ggreg2(fit, "pH_s_ptb_Tptb", "pHcalc_ptB") +
  geom_line(aes(x = pH_s_ptb_Tptb, y = pH_s_ptb_Tptb), linetype = 2)  +
  geom_point(data=ptb_noNAs%>%filter(date == "2018-01-18"), aes(x = pH_s_ptb_Tptb, y = pHcalc_ptB),colour="red", size=3) +
  mytheme+labs(x="pH Spec", y="pH Calc") +
...
ggsave("../../pCloud Sync/exp166_seafet/1b_seafet_pointB/pb_figures/all_figures/comparaison_pHcalc_pHsepc.png",comparaison_p,height =18, units="cm")
...
Following are dates and times (in UTC) for every lowering and recovery (deployement period) of the sensor. Changement of batteries and any other problems are also added here:
  ...
  -  **2019-03-29:** back in water at 09:30.
  -  **2019-03-27:** out of water at 09:10 to give to christophe and Jean-Michel for tests.
