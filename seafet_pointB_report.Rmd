...
file_list <- list.files(path  = "./../pb_data/Individual_data/", pattern = "Raw_seaFET2.*.csv")

if (length(file_list) == 0) {stop()} # script does not run if there is no data file to read
ii <- 0
for (file in file_list){ 
  ii <- ii + 1
  tmp <- read.table(paste("./../pb_data/Individual_data/", file, sep = ""), header = F, dec = ".", as.is = T, sep = ";", skip = 1, fill = TRUE)
...
if (file.exists("../pb_data/all_data/seaFET_ptb.Rdata") == TRUE) {
  load("../pb_data/all_data/seaFET_ptb.Rdata")
  z_old <- as_tibble(TAB)
}
...
y <- read.table("../pb_data/all_data/Data_DIC.csv", header=T, sep=";", dec=".", as.is=T)
...
write.table(TAB,"../pb_data/all_data/time-series.csv",row.names = FALSE, sep=",", dec=".")
```
...
![ ](../../3_Images_seafet/2017-01-10_seafet_DeLiege/DSC01795.jpg){width=250px}
...
x <- read.table("../../8_Intercomparaison/spectro.csv", header=T, sep=",", dec=".", as.is=T)
...
write.table(discrete,"../../8_Intercomparaison/discrete_data_insitu_PROCESSED.csv",row.names=FALSE,sep=",",dec=".")
...
write.table(TAB,"../pb_data/all_data/ALLdata_discrete_continuous.csv",row.names=FALSE,sep=",",dec=".")
...
TAB <- read.table("../pb_data/all_data/ALLdata_discrete_continuous.csv", header=T, sep=",", dec=".", as.is=T)
...
ggsave("../pb_figures/all_figures/time_series_cont.png",time_series_cont,height =18, units="cm")
...
time_series_dis <- ggplot(TABm_hole_dis) + geom_point(aes(x=date, y=value, color=variable),size=0.5) +
  ...
  theme_bw()+ xlab("Time")+ylab("")+
  scale_colour_discrete(guide="none")
ggsave("../pb_figures/all_figures/time_series_dis.png",time_series_dis,height =18, units="cm")
```
...
discrete_ptb <- read.table("../../8_Intercomparaison/discrete_data_insitu_PROCESSED.csv",header=T,sep=",",dec=".")
...
ggsave("../pb_figures/calib_E0ext25_E0int25_ptb.png",p_calib_ptb,height =20, units="cm")
...
write.table(discrete_ptb1,"../pb_data/all_data/continuous_discrete.csv",row.names=FALSE,sep=",",dec=".")

# Do it on all seafet data
seafet_ptb <- read.table("../pb_data/all_data/time-series.csv",header=T,sep=",",dec=".")
...
write.table(seafet_all_ptb,"../pb_data/all_data/continuous_discrete.csv",row.names=FALSE,sep=",",dec=".")
...
comparaison <- read.table("../pb_data/all_data/Data_DIC.csv", header=T, sep=";", dec=".", as.is=T)
...
# interolation of missing TA and nutrients
...
comparaison_p <-  
  ...
  mytheme+labs(x="pH PointB", y="pH EOL") +
   scale_x_continuous(breaks = seq(7.95,8.15,0.01))  +
   scale_y_continuous(breaks = seq(7.95,8.15,0.01)) 
